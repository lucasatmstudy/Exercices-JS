let prenom = "Marie";
let age = 30;
let ville = "Paris";
let blague = "Qu\'est-ce qu\'une pizza a dit à une autre pizza qui lui demandait des conseils? \"Suis ta pâte et tout ira bien!\""

let commande = {
    article: 2,
    pizza: "calzone",
    prix: 12.5 + 12.5,
    adresse: "11 avenue de l'europe",
    temps: "30",
};

console.log("Bonjour", prenom, "votre commande :",commande.pizza, "est en cours de préparation.","\n","Elle vous sera livrée d'ici", commande.temps, "minutes à votre adresse", commande.adresse, ".")
console.log("Prix total :", commande.prix, "euros")
console.log("Merci d\'avoir commandé chez la pizzerria \"Rafinata\".","\n",blague)